...
def objective(dataset, config, *, input_scaling, N, sr, lr, ridge, seed, connectivity, input_connectivity,noise_rc):
    ...
    for n in range(instances):
        ...
        split_index = int(50000*0.5)
        # Train your model and test your model.
        model.fit(X[:split_index], Y[:split_index], warmup=500) 

        #######################################
        orientation = np.array(Data_set[split_index, 10])
        position = np.array([Data_set[split_index, 8],
                             Data_set[split_index, 9]]).reshape((1, 2))
        
        # derive starting zone from actual position
        start_zone = get_zones(position)
        zone = start_zone
        
        # find matching index in pattern
        # check which occurrence is correct by looking at next zone transition
        for i in range(split_index, min(split_index + 500, len(Data_set))):
            pos = Data_set[i, 8:10]
            if pos[1] > 350:
                next_zone = 1
            elif pos[1] < 150:
                ...
                next_zone = 3
            if next_zone != start_zone:
                break
        
        # find idx such that pattern[idx] == start_zone 
        # and pattern[(idx+1)%6] == next_zone
        idx = 0
        for i in range(6):
            if pattern[i] == start_zone and pattern[(i+1)%6] == next_zone:
                idx = i
                break
        
        print(f"Starting zone: {start_zone}, next zone: {next_zone}, idx: {idx}")
        ######################################
        ...
        ############
        #orientation=np.array(Data_set[split_index,10]) # initialisation of bot's orientation
        #position=np.array([Data_set[split_index,8],Data_set[split_index,9]]).reshape((1,2)) # bot's position
        
        pred=int(50000*0.5) # number of predictions for the test
